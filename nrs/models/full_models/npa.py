...
class MultiInterestNPA(nn.Module):
    def __init__(self, cfg, rec_model):
        super(MultiInterestNPA, self).__init__()
        self.user_embedder = nn.Embedding(
            num_embeddings=cfg.n_users + 1,  # 0 for padding
            embedding_dim=cfg.user_emb_dim
        )
        ...
        # Multi-Interest User Encoder with query features
        ...
        self.user_attention = layers.PersonalizedAttention(
            in_features=cfg.title_emb_dim,
            hidden_features=128,
            query_features=cfg.user_emb_dim
        )
    ...
    def _forward(self, 
        ...
        # Moving to device should not be done here
        device = next(self.parameters()).device
        ...
        # User encoding with multi-interests
        b, nh, s, d = h.shape
        h = h.reshape((b * nh, s, d))
        hm_title = hm.reshape((b * nh, s, 1))
        h = self.dropout(h)
        hist_user_emb = user_emb.repeat_interleave(nh, dim=0)  # (b * nh, du)
        h = self.title_pooler(q=hist_user_emb, x=h, m=hm_title)  # (b * nh, d)
        h = self.news_head(h)  # (b * nh, d_emb)
        emb_dim = h.shape[1]
        h = h.reshape((b, nh, emb_dim))  # (b, nh, d_emb)
        hm = collaps_mask(hm, dim=2)  # (b, nh)

        # Encode user interests using MultiInterestUserEncoder
        user_interests = self.user_encoder((h, hm))

        # Aggregate user interests using PersonalizedAttention
        u = self.user_attention(q=user_emb, x=user_interests, m=None)  # (b, 1, e_emb)
