...
class MultiInterestCAUMUserEncoder(nn.Module):
    '''MultiInterestCAUMUserEncoder:

    Purpose:
    MultiInterestCAUMUserEncoder encodes user behavior using CNN operations, self-attention,
    and dense attention mechanisms. It outputs a matrix representing the user's multiple interests.

    Initialization:
    Initializes the user encoder with CNN layers, self-attention, dense attention, specified poolers,
    dropout rates, embedding dimensions, and bias terms.

    Arguments:
    - cfg: Configuration parameters for the model.
    '''

    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        # Initialize layers and parameters for multi-interest user representation
        self.dropout1 = nn.Dropout(p=cfg.p_dropout)
        self.dropout2 = nn.Dropout(p=cfg.p_dropout)
        self.dropout3 = nn.Dropout(p=cfg.p_dropout)
...
class MultiInterestCAUM(nn.Module):
    '''MultiInterestCAUM:

    Purpose:
    MultiInterestCAUM integrates text embeddings, category information, and multi-interest user representations
    to generate recommendations. It leverages CNN-based operations, self-attention, and dense layers for feature
    processing and aggregation.

    Initialization:
    Initializes the CAUM model with the provided configuration parameters and the specified recommendation model.

    Arguments:
    - cfg: Configuration parameters for the model.
    - rec_model: Recommendation model for generating recommendations.
    '''
    ...
    def __init__(self, cfg, rec_model: nn.Module):
        ...
        self.cfg = cfg
        self.news_encoder = CAUMNewsEncoder(cfg)
        self.user_encoder = MultiInterestCAUMUserEncoder(cfg)  # Use MultiInterestCAUMUserEncoder
        self.rec_model = rec_model
