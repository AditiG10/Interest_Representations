...
class StandardRec(SingleTextParentRec):
...
class StandardRecMultiInterest(MultiInterestParentRec):
    def __init__(self, cfg, scoring_fn: nn.Module):
        # Title Encoder for news
        news_encoder = TextEncoder(
            pooler=AdditiveAttention(cfg.d_backbone, 256),
            p_dropout=cfg.p_dropout,
            out_features=cfg.title_emb_dim,
            in_features=cfg.d_backbone,
            bias=cfg.bias
        ...
        super(StandardRecMultiInterest, self).__init__(
