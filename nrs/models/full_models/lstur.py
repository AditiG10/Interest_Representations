...
from ..components import layers, news_encoding
...
class LSTURMultiInterestUserEncoder(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
...
class LSTURMultiInterest(nn.Module):
    def __init__(self, cfg, rec_model):
        super().__init__()
        self.news_encoder = LSTURNewsEncoder(cfg)
        self.user_encoder = LSTURMultiInterestUserEncoder(cfg)  # Updated to use multi-interest user encoder
        self.rec_model = rec_model
        self.cfg = cfg
    ...
    def _forward(self,
        ...
        # Use multi-interest user encoder to encode user history features
        u = self.user_encoder(h, hm, user_ids)
