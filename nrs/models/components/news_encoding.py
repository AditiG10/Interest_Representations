...
class TextEncoder(nn.Module):
    ...
    def forward(self, inpt: tuple):
        ...
        m = m.to(device)
        b, n, s, d = x.shape
        x = x.reshape((b * n, s, d)) # Reshape x to (B*N, S, D)
        m = m.reshape((b * n, s, 1)) # Reshape m to (B*N, S, 1)
        x = self.dropout(x)
        ...
        #Output Transformation- Head
        if hasattr(self, 'head'):
            x = self.head(x)

        #Reshape and return
        x = x.reshape((b, n, self.out_dim)) # Reshape x to (B, N, out_features)
        m = m.reshape((b, n, s, 1)) # Reshape m back to (B, N, S, 1)
        m = collaps_mask(m, dim=2) # Collapse mask to (B, N, 1)
        return (x, m)    #Output shape (B, N, out_features), (B, N, 1)   
