...
# TODO: change to pytorch metrics
...
def false_mrr_score(y_true, y_score):
    ...
    highest_rr = np.max(rr_score)
    return highest_rr if np.sum(y_true) > 0 else 0.0
...
def rr_score(y_true, y_score):
    ...
    y_true_sorted = np.take(y_true, order)
    rr_score = y_true_sorted / (np.arange(len(y_true_sorted)) + 1)
    return np.max(rr_score) if np.sum(y_true) > 0 else 0.0
